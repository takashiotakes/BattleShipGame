...
const App: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      {step === "select" ? (
        <PlayerSelector
          players={players} // ★問題の可能性あり★ この props は不要になりました
          onPlayersChange={handlePlayersChange} // ★問題の可能性あり★ この props は不要になりました
          onNext={handleStartPlacement}
        />
      ) : (
        <ShipPlacement players={players} /> // ★問題の可能性あり★ この props は不要になりました
      )}
    </div>
  );
};
